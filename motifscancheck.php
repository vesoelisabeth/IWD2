<?php // No spaces or newlines before this
session_start();
...
// Debug log file
$debug_log = '/home/s2015320/public_html/project2/motif_debug.log';
file_put_contents($debug_log, "Script started\n", FILE_APPEND);

// Define output file
$temp_output = "/home/s2015320/public_html/project2/motif_scan_" . session_id() . ".out";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    file_put_contents($debug_log, "Sequence data received: " . json_encode($sequence_data) . "\n", FILE_APPEND);

    // Store sequence data in session to preserve it
    $_SESSION['last_sequence_data'] = json_encode($sequence_data);

    // Prepare FASTA and run patmatmotifs per sequence
    $combined_output = "";
    $motifs = [];
    foreach ($sequence_data['sequences'] as $seq) {
        ...
        file_put_contents($debug_log, "FASTA Input for {$seq['id']}:\n$fasta_input\n", FILE_APPEND);

        $temp_single_output = "/home/s2015320/public_html/project2/temp_motif_" . uniqid() . ".out";
        $command = "/usr/bin/patmatmotifs -sequence " . escapeshellarg($temp_fasta) . " -full -noprune -sformat fasta -rformat simple -outfile " . escapeshellarg($temp_single_output) . " 2>&1";
        $result = shell_exec($command);
        file_put_contents($debug_log, "Command: $command\nResult: $result\n", FILE_APPEND);

        if (file_exists($temp_single_output) && filesize($temp_single_output) > 0) {
            $output_content = file_get_contents($temp_single_output);
            $combined_output .= $output_content . "\n";

            // Parse motifs from this sequence
            $lines = explode("\n", $output_content);
            $current_seq = '';
            $feature_data = [];
            foreach ($lines as $line) {
                ...
                if (preg_match('/^#\s+Sequence:\s+(\S+)/', $line, $matches)) {
                    ...
                    file_put_contents($debug_log, "Parsing sequence: $current_seq\n", FILE_APPEND);
                    ...
                    if (isset($feature_data['name'], $feature_data['start'], $feature_data['end'], $feature_data['motif']) && $current_seq) {
                        $motifs[] = [
                        ...
                        file_put_contents($debug_log, "Added motif for $current_seq: {$feature_data['motif']}\n", FILE_APPEND);
                    }
                }
            }
            unlink($temp_single_output);
        } else {
            file_put_contents($debug_log, "Motif scan failed for {$seq['id']}: $result\n", FILE_APPEND);
        }
        unlink($temp_fasta);
    }

    // Write combined output
    file_put_contents($temp_output, $combined_output);
    chmod($temp_output, 0644); // Ensure readable
    file_put_contents($debug_log, "Combined Output written to $temp_output:\n$combined_output\nMotifs: " . print_r($motifs, true) . "\n", FILE_APPEND);
} else {
    // For GET requests, load motifs from last run
    if (file_exists($temp_output)) {
...
<body>
    ...
    <div>
        ...
        <?php endif; ?>
        <p>
            <a href="/~s2015320/public_html/project2/motifscan.php?download=report"><button>Download Report</button></a>
        </p>
        <form action="/~s2015320/public_html/project2/analysis_tool.php" method="post">
