...
OUTPUT_DIR = f"/tmp/advanced_analysis_{SESSION_ID}/"
...
def generate_phylogeny(alignment_file):
    try:
        ...
        plt.close()
        return "phylogeny.png"  # Relative path
    except Exception as e:
        return f"Error: {e}"
...
def generate_report(analysis_results):
    html_lines = [
        ...
        "</ul>",
        "<h2>Phylogeny</h2>",
        f'<img src="{analysis_results["phylogeny"]}" width="50%">' if os.path.exists(os.path.join(OUTPUT_DIR, analysis_results["phylogeny"])) else f"No phylogeny: {analysis_results['phylogeny']}",
        "</body></html>"
    ]
    html = "\n".join(html_lines)
